...
function ciniki_materiamedica_plantImageUpdate(&$ciniki) {
    ...
    $rc = ciniki_materiamedica_checkAccess($ciniki, $args['tnid'], 'ciniki.materiamedica.plantImageUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    if( !isset($rc['item']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.materiamedica.15', 'msg'=>'Plant image not found'));
    }
    $item = $rc['item'];
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.materiamedica.16', 'msg'=>'You already have an image with this name, please choose another name'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.materiamedica.plant_image', $args['plant_image_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['primary_image']) ) {
        if( $args['primary_image'] == 'yes' ) {
            $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.materiamedica.plant', $item['plant_id'], array('image_id'=>$image_id), 0x04);
            if( $rc['stat'] != 'ok' ) {
                return $rc;
            }
            ...
            if( isset($rc['plant']['image_id']) && $rc['plant']['image_id'] == $image_id ) {
                $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.materiamedica.plant', $item['plant_id'], array('image_id'=>0), 0x04);
                if( $rc['stat'] != 'ok' ) {
                    return $rc;
                }
            }
